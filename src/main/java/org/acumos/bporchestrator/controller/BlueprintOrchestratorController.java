...
public class BlueprintOrchestratorController {
	...
	public ResponseEntity<byte[]> notify(
		...
		// On receiving this request, the data source is blocked. We will need to unblock it only when we
		...
		try {
			...
			/*for (InputPort inport : inps) {
				if (inport.getOperationSignature().getOperationName().equals(operation)) {
					validInputOperation = true;
					break;
				}
			}
			if (validInputOperation == false) {
				logger.error(
			...
			if ((dataBrokerPresent == true) && (databrokervisited == false)) {
				...
				if (probePresent == true) {
					...
					// triggers.)
					logger.info("Notifying PROBE for node name" + dbnode.getContainerName() + ", inp msg name:" + msgName + ", msg: "+ binaryStream);
				 ...
				 */
				if (probePresent == true) {
					logger.info("Notifying PROBE for node name " + inpnode.getContainerName() + ", inp msg name:" + inpmsgname + ", msg: "+ binaryStream);
	...
	public byte[] notifynextnode(byte[] output, Node n, String oprn) {
		...
		for (ConnectedTo depitem : deps) {
			...
			//find dependents of the current dependent to see if we need to send input msg or bothe nput and output msg.
			try {
			...
			if (probePresent == true) {
				...
				if ( !(depsofdeps.isEmpty())) {
					try {
						logger.info("Notifying PROBE for node name" + depcontainername + ", inp msg name:" + depinpmsgname + ", msg: "+ depoutput);
				...
				if (depsofdeps.isEmpty() && !(n.getNodeType().equalsIgnoreCase("Probe"))) {
					...
					try {
						logger.info("Notifying PROBE for node name" + depcontainername + ", out msg name:" + depoutmsgname + ", msg: "+ depoutput);
